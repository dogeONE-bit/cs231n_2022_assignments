    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        for i in range(max_len):
          for j in range(embed_dim):
            if(j%2==0):
              pe[0,i,j]=math.sin(i*10000**(-1*j/embed_dim))
            else:
              pe[0,i,j]=math.cos(i*10000**(-1*(j-1)/embed_dim))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        key=self.key(key).reshape(N,T,H,E//H).permute(0,2,1,3)
        value=self.value(value).reshape(N,T,H,E//H).permute(0,2,1,3)
        A=torch.matmul(query,key.permute(0,1,3,2))/torch.sqrt(torch.tensor(E//H))
        if attn_mask is not None:
            A = A.masked_fill(attn_mask == 0, -float('inf'))
        A=self.attn_drop(sft(A))
        output=self.proj(torch.matmul(A,value).permute(0,2,1,3).reshape(N,S,E))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
