...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h=h0
    for i in range(T):
      next_h,cache_new=rnn_step_forward(x[:,i,:],prev_h,Wx,Wh,b)
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
      dx[:,i,:], dh_t, dWx_t, dWh_t, db_t=rnn_step_backward(dh[:,i,:] if i==T-1 else dh[:,i,:]+dh_t, cache.pop())
      dWx+=dWx_t
      dWh+=dWh_t
      db+=db_t
    dh0=dh_t
...
def lstm_backward(dh, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N,T,H=dh.shape
    x,_,_,Wx,Wh,b,_,_,_,_,_,_=cache[-1]
    D=x.shape[1]
    dx=np.zeros((N,T,D))
    ...
    db=np.zeros_like(b)
    for i in reversed(range(T)):
      dx[:,i,:],dprev_h,dprev_c,dWx_t,dWh_t,db_t=lstm_step_backward(dh[:,i,:] if i==T-1 else dh[:,i,:]+dnext_h,dnext_c,cache.pop())
      dnext_h=dprev_h
      dnext_c=dprev_c
      dWx+=dWx_t
      dWh+=dWh_t
