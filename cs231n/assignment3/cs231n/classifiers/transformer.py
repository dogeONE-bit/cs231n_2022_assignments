...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        captions_emd=self.positional_encoding(self.embedding(captions))#N(N,T,W)
        features_proj=self.visual_projection(features).unsqueeze(1)#(N,W)->N(N,1,W)
        tgt_mask=torch.tril(torch.ones(T,T))
        features = self.transformer(tgt=captions_emd,memory=features_proj,tgt_mask=tgt_mask)
