...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for t in range(N):
          if(t!=k):
            sum_k_kN+=torch.exp(sim(z_k,out[t])/tau)+torch.exp(sim(z_k,out[t+N])/tau)
            sum_kN_k+=torch.exp(sim(z_k_N,out[t])/tau)+torch.exp(sim(z_k_N,out[t+N])/tau)
        sum_k_kN+=torch.exp(sim(z_k,z_k_N)/tau)
        sum_kN_k+=torch.exp(sim(z_k_N,z_k)/tau)
        total_loss+=-2*sim(z_k,z_k_N)/tau+torch.log(sum_k_kN)+torch.log(sum_kN_k)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    pos_pairs=torch.diag(torch.matmul(out_left,out_right.T))/(torch.linalg.norm(out_left,dim=1)*torch.linalg.norm(out_right,dim=1))
...
def compute_sim_matrix(out):
    ...
    sim_matrix=torch.matmul(out,out.T)/(torch.matmul(torch.linalg.norm(out,dim=1).unsqueeze(1),torch.linalg.norm(out,dim=1).unsqueeze(0)))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    #pos_pairs=sim_matrix[torch.arange(N),torch.arange(N)+N].unsqueeze(1)
